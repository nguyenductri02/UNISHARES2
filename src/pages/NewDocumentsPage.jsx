...
import { FaSearch, FaDownload, FaEye, FaUser, FaCalendarAlt } from 'react-icons/fa';
...
const NewDocumentsPage = () => {
  ...
  const [filters, setFilters] = useState({
    subject: '',
    sortBy: 'latest' // Default sort by latest for new documents
  });
  ...
  const fetchNewDocuments = async () => {
    try {
      ...
      const params = {
        ...filters,
        search: searchQuery,
        sort: 'latest' // Always sort by latest for new documents
      };
      
      const response = await documentService.getNewDocuments(params);
  ...
  return (
    <>
      ...
      <Container className="py-4">
        <h1 className="mb-4">Tài liệu mới</h1>
                ...
                <Card className="h-100 shadow-sm hover-card">
                  <Card.Body>
                    ...
                    <Card.Text className="small text-muted mb-3">
                      <FaCalendarAlt className="me-1" /> {document.created_at ? formatDate(document.created_at) : 'Không xác định'}
                    </Card.Text>
